...
integer*4 :: patch_side,patch_nums ! patch_size
integer*4 :: ens_num !number of ensembles
real*4 :: res !model horizontal resolution
...
real, allocatable :: excludedGrids(:,:),localExc_line(:) !excluded grids from observation [user-defined]
...
subroutine main_assimilation
    !main iteration for all grids
    integer*4 :: countnum,i,j,k
    real, allocatable :: xf_m(:),localx(:,:,:),localx_line(:) !raw simulation
    real, allocatable :: local_obs_line(:)
    real, allocatable :: local_line_ocean(:) !lsmask (1=land,0=ocean)
    real, allocatable :: local_line_exc(:) !excluded grids from observation [user-defined]
    implicit none

    ! parallel calculation by openmp
    !$omp parallel default(shared) private(lat,lon,lat_cent,xt,local_line_ocean,countnum,increment,res,nLon,nLat, &
    !$omp& local_obs_line,localx,localx_line,xf,local_line_exc,i,j,k,xf_m,H,errflg,xa_m,Ef,ovs,R,Rdiag,W,VDVT,Pa, &
...
end subroutine main_assimilation
