...
class LETKF_core(object):
    ...
    required_instance_vals = ['nLon', 'east', 'assimE', 'nLat', 'sysDir', 'res', 'patch', 'assimS', 'west', 'assimW', 'srcDir', 'north', 'errexp', 'assimN', 'ensMem', 'south', 'undef', 'errfix']
    ...
    def __init__(self,configPath=None):
        ...
        if type(configPath) == str and os.path.exists(configPath):
            ...
            self.errexp = float(config.get("assimilation","errExp"))
            self.srcDir = config.get("model","srcDir")
            ...
            self.errfix = float(config.get("observation","errfix"))
            self.sysDir = config.get("system","sysDir")
    ...
    def letkf(self,ensembles,observation,ocean,excGrids):
        """
        ...
        # main letkf
        xa = letkf.letkf(ensembles,observation,ocean,excGrids,self.patch,self.ensMem,self.assimE,self.assimW,self.assimN,self.assimS,self.east,self.west,self.north,self.south,self.res,self.undef,self.errfix)
